...
init dbFileName = do
        ...
        (ver : _) : _ <-
                Sql.query_ conn "select sqlite_version()" :: IO [[String]]
        putStrLn $ "sqlite version " <> ver
        Sql.execute_
                conn
                "create table if not exists orders (id integer primary key, amount integer, email text, address text, delivery_time text)"
        pure conn


addOrder :: Sql.Connection -> T.OrderBody -> IO ()
addOrder conn order = Sql.execute
        conn
